...
export default async function SuppliersAnalytics() {
  ...
  try {
    ...
    trend = await get7DayTrend(db, 'suppliers');
  } catch (e) {
  ...
  try {
    const { db } = await connectToDatabase();
    total = await db.collection('suppliers').countDocuments();
    try { active = await db.collection('suppliers').countDocuments({ isActive: true }); } catch { active = 0; }
  } catch (e) { }
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        <div className="p-3 bg-white/5 border border-neutral-800 rounded-lg">
          <div className="text-xs text-neutral-400">Total Suppliers</div>
          <div className="text-lg font-semibold">{total}</div>
        </div>
        <div className="p-3 bg-white/5 border border-neutral-800 rounded-lg">
          <div className="text-xs text-neutral-400">Active Suppliers</div>
          <div className="text-lg font-semibold">{active}</div>
        </div>
        <div className="p-3 bg-white/5 border border-neutral-800 rounded-lg">
          <div className="text-xs text-neutral-400">Active %</div>
          <div className="text-lg font-semibold">{total ? Math.round((active/total)*100) : 0}%</div>
        </div>
      </div>
